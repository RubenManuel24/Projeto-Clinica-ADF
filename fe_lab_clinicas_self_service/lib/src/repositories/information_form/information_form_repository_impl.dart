  ...
  Future<Either<RepositoryException, Unit>> register(
    ...
    try {
      final SelfServiceModel(
        :name!,
        :lastName!,
        patient: PatientModel(id: patirntId)!,
        documents: {
          DocumentType.healthImsuranceCard: List(first: healthImsuranceCardDoc),
          DocumentType.medicalOrder: medicalOrderDocs,
        }!
      ) = model;

      await restClient.auth.post('/patientInformationForm', data: {
        'patient_id': patirntId,
        'health_insurance_card': healthImsuranceCardDoc,

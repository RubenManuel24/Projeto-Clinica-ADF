...
import 'package:fe_lab_clinicas_adm/src/repositories/patient_information_form_repository/patient_information_form_repository.dart';
...
class CheckinController with MessageStateMixin {
  ...
  Future<void> endCheckin() async {
    if (informationForm() != null) {
      final result = await _patientInformationFormRepository.updateStatus(informationForm.value!.id,
          PatientInformationFormStatus.beingAttended);
      switch(result){
        case Left():
          showError('Erro ao atualizar o status do formúlario');
        case Right():
